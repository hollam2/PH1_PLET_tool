...
rm(dates)
```
Replace polygon coordinates with a unique identifier (automatically detect if it is multipolygon data)
```{r}
df <- create_assess_id(x=df)
```
Add 0.5* the min value for the lifeform (method=1) or 1 (method=2) and log10 transform the data
```{r}
