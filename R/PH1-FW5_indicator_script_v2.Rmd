...
This tool uses a csv fileoutput from the Plankton Lifeform Extraction Tool (PLET) available on DASSH: https://www.dassh.ac.uk/lifeforms/
...
df <- head(fread(paste0(dir_data, file_lf)),-1) %>%
  do(janitor::clean_names(.)) %>%
  do(as.data.frame(.))
```
Generate a string for the lifeforms contained in the dataset
```{r}
id_vars <- c("period", "num_samples", "abundance_type", "taxa_used")
